...
bool Compositor::Draw(DisplayPlaneStateList &comp_planes,
  ...
  for (DisplayPlaneState &plane : comp_planes) {
    if (plane.GetCompositionState() == DisplayPlaneState::State::kScanout) {
      ...
      Render(layers, in_flight_surfaces_.back(), comp_regions, plane.GetLeft(),
             plane.GetTop(), plane.GetRight(), plane.GetBottom());
...
bool Compositor::DrawOffscreen(std::vector<OverlayLayer> &layers,
  ...
  Render(layers, surface.get(), comp_regions, 0, 0, width_, height_);
...
void Compositor::AddOutputLayer(std::vector<OverlayLayer> &layers,
                                NativeSurface *surface, uint32_t left,
  ...
  pre_comp_layer.SetSourceCrop(HwcRect<float>(left, top, right, bottom));
  pre_comp_layer.SetDisplayFrame(HwcRect<int>(left, top, right, bottom));
...
void Compositor::Render(std::vector<OverlayLayer> &layers,
  ...
  AddOutputLayer(layers, surface, left, top, right, bottom);
}
